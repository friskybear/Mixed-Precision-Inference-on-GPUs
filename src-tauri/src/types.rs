use crate::kernel::{BIAS_RELU_FP16_KERNEL, BIAS_RELU_FP32_KERNEL, FP16_KERNEL, FP16_SCALED_KERNEL, FP16_TO_FP32_KERNEL, FP32_KERNEL};
use crate::{
    CLBLAST_LAYOUT_ROW_MAJOR, CLBLAST_TRANSPOSE_NO, CLBLAST_TRANSPOSE_YES, f16_to_f32, f32_to_f16, load_clblast, pick_device
};
...
pub type CLBlastHgemmFn = unsafe extern "C" fn(
    layout: i32,
    a_transpose: i32,
    b_transpose: i32,
    m: usize,
...
pub static MLP_INSTANCE: Mutex<Option<MLPInference>> = Mutex::new(None);
...
impl MLPInference {
    ...
    pub fn warmup_clblast(&mut self, matrix_size: usize) {
    ...
    pub fn run_fp32_inference(&mut self, rd: &RoundData) -> Result<InferenceMetrics, String> {
        ...
        unsafe {
            ...
            // Actual bytes read + written during kernel execution:
            // Layer 1 reads: input(B*I) + weights1(H*I) + bias1(H), writes: hidden(B*H)
            // Layer 2 reads: hidden(B*H) + weights2(O*H) + bias2(O), writes: output(B*O)
            let memory_transferred = ((batch_size * input_size
                + hidden_size * input_size
                + hidden_size
                + batch_size * hidden_size
                + batch_size * hidden_size
    ...
    pub fn run_fp16_inference(&mut self, rd: &RoundData) -> Result<InferenceMetrics, String> {
        let input_size = rd.input_size;
        let hidden_size = rd.hidden_size;
        let output_size = rd.output_size;
        let batch_size = rd.batch_size;
        ...
        unsafe {
            ...
            // Actual bytes read + written during kernel execution (all FP16 = 2 bytes):
            // Layer 1 reads: input(B*I) + weights1(H*I) + bias1(H), writes: hidden(B*H)
            // Layer 2 reads: hidden(B*H) + weights2(O*H) + bias2(O), writes: output(B*O)
            let memory_transferred = ((batch_size * input_size
                + hidden_size * input_size
    ...
    pub fn run_fp16_scaled_inference(
        &mut self,
        rd: &RoundData,
        ...
        unsafe {
            ...
            // Calculate metrics - FP16 weights/input/bias/output + FP32 scales + FP32 hidden
            let fp16_bytes = (batch_size * input_size
                + hidden_size * input_size
                + hidden_size
                + output_size * hidden_size
                + output_size
                + batch_size * output_size)
                * 2;
            let fp32_bytes = (hidden_size + output_size   // scales
                + batch_size * hidden_size)               // hidden buffer is FP32
                * 4;
            let memory_footprint_mb = (fp16_bytes + fp32_bytes) as f64 / (1024.0 * 1024.0);
            ...
            // Actual bytes read + written during kernel execution:
            // Layer 1 reads (FP16): input(B*I), weights1(H*I), bias1(H); reads (FP32): scales1(H)
            // Layer 1 writes (FP32): hidden(B*H)
            // Layer 2 reads (FP32): hidden(B*H), scales2(O); reads (FP16): weights2(O*H), bias2(O)
            // Layer 2 writes (FP16): output(B*O)
            let transferred_fp16 = (batch_size * input_size
                + hidden_size * input_size
                + hidden_size
                + output_size * hidden_size
                + output_size
                + batch_size * output_size)
                * 2;
            let transferred_fp32 = (hidden_size            // scales1 read
                + batch_size * hidden_size                 // hidden write
                + batch_size * hidden_size                 // hidden read
                + output_size)                             // scales2 read
                * 4;
            let memory_transferred = (transferred_fp16 + transferred_fp32) as f64;
            let memory_bandwidth_gbps = memory_transferred / (kernel_time.as_secs_f64() * 1e9);
    ...
    // ==================== CLBlast FP32 (SGEMM) ====================
    pub fn run_clblast_fp32_inference(
        &mut self,
        rd: &RoundData,
    ) -> Result<InferenceMetrics, String> {
        ...
        unsafe {
            ...
            let total_flops = (batch_size
                * (hidden_size * (2 * input_size + 1) + output_size * (2 * hidden_size + 1)))
                as f64;
            let throughput_gflops = total_flops / (kernel_time.as_secs_f64() * 1e9);
            // Actual bytes read + written during kernel execution (all FP32 = 4 bytes):
            // SGEMM L1 reads: input(B*I) + weights1(H*I), writes: hidden(B*H)
            // Bias+ReLU reads+writes: hidden(B*H), reads: bias1(H)
            // SGEMM L2 reads: hidden(B*H) + weights2(O*H), writes: output(B*O)
            // Bias reads+writes: output(B*O), reads: bias2(O)
            let memory_transferred = ((batch_size * input_size
                + hidden_size * input_size
                + batch_size * hidden_size
                + batch_size * hidden_size
    ...
    // ==================== CLBlast FP16 (HGEMM) ====================
    pub fn run_clblast_fp16_inference(
        &mut self,
        rd: &RoundData,
    ) -> Result<InferenceMetrics, String> {
        let clblast = self.clblast.as_ref().ok_or("CLBlast not loaded")?;
        ...
        unsafe {
            ...
            let throughput_gflops = total_flops / (kernel_time.as_secs_f64() * 1e9);
            // Actual bytes read + written during kernel execution (all FP16 = 2 bytes):
            // HGEMM L1 reads: input(B*I) + weights1(H*I), writes: hidden(B*H)
            // Bias+ReLU reads+writes: hidden(B*H), reads: bias1(H)
            // HGEMM L2 reads: hidden(B*H) + weights2(O*H), writes: output(B*O)
            // Bias reads+writes: output(B*O), reads: bias2(O)
            let memory_transferred = ((batch_size * input_size
                + hidden_size * input_size
                + batch_size * hidden_size
    ...
    pub fn run_clblast_mixed_inference(
        &mut self,
        rd: &RoundData,
    ) -> Result<InferenceMetrics, String> {
        let clblast = self.clblast.as_ref().ok_or("CLBlast not loaded")?;
        ...
        unsafe {
            ...
            let throughput_gflops = total_flops / (kernel_time.as_secs_f64() * 1e9);
            // Actual bytes read + written during kernel execution:
            // Convert reads (FP16): input(B*I) + weights1(H*I), writes (FP32): input(B*I) + weights1(H*I)
            // SGEMM L1 reads (FP32): input(B*I) + weights1(H*I), writes (FP32): hidden(B*H)
            // Bias+ReLU reads+writes (FP32): hidden(B*H), reads: bias1(H)
            // Convert reads (FP16): weights2(O*H), writes (FP32): weights2(O*H)
            // SGEMM L2 reads (FP32): hidden(B*H) + weights2(O*H), writes (FP32): output(B*O)
            // Bias reads+writes (FP32): output(B*O), reads: bias2(O)
            let transferred_fp16_read =
                (batch_size * input_size + hidden_size * input_size + output_size * hidden_size)
                    * 2;
            let transferred_fp32 = (
                // convert writes
                batch_size * input_size + hidden_size * input_size
                // SGEMM L1 reads + writes
                + batch_size * input_size + hidden_size * input_size + batch_size * hidden_size
                // bias+relu read+write + bias1 read
                + batch_size * hidden_size + batch_size * hidden_size + hidden_size
                // convert write
                + output_size * hidden_size
                // SGEMM L2 reads + writes
                + batch_size * hidden_size + output_size * hidden_size + batch_size * output_size
                // bias read+write + bias2 read
                + batch_size * output_size + batch_size * output_size + output_size
            ) * 4;
            let memory_transferred = (transferred_fp16_read + transferred_fp32) as f64;
            let memory_bandwidth_gbps = memory_transferred / (kernel_time.as_secs_f64() * 1e9);
